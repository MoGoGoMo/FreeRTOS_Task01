/**
  ******************************************************************************
  * @file    Project/F446_T1/bsp_usart_fifo.c
  * @author  Gordon Tao
  * @version V0.1
  * @date    27-June-2019
  * @brief   Source file for USART  module
  ******************************************************************************
  * @attention
  * @Modified_log:
...
/* Define the USART Structure for each one */
#if UART1_FIFO_EN == 1
	static UartStr_TypeDef g_tUart1;
	static uint8_t g_TxBuf1[UART1_TX_BUF_SIZE];		
	static uint8_t g_RxBuf1[UART1_RX_BUF_SIZE];		
#endif

#if UART2_FIFO_EN == 1
	static UartStr_TypeDef g_tUart2;
	static uint8_t g_TxBuf2[UART2_TX_BUF_SIZE];		
	static uint8_t g_RxBuf2[UART2_RX_BUF_SIZE];		
#endif

#if UART3_FIFO_EN == 1
	static UartStr_TypeDef g_tUart3;
	static uint8_t g_TxBuf3[UART3_TX_BUF_SIZE];		
	static uint8_t g_RxBuf3[UART3_RX_BUF_SIZE];		
#endif

#if UART4_FIFO_EN == 1
	static UartStr_TypeDef g_tUart4;
	static uint8_t g_TxBuf4[UART4_TX_BUF_SIZE];		
	static uint8_t g_RxBuf4[UART4_RX_BUF_SIZE];		
#endif

#if UART5_FIFO_EN == 1
	static UartStr_TypeDef g_tUart5;
	static uint8_t g_TxBuf5[UART5_TX_BUF_SIZE];		
	static uint8_t g_RxBuf5[UART5_RX_BUF_SIZE];		
#endif

#if UART6_FIFO_EN == 1
	static UartStr_TypeDef g_tUart6;
...
static void UartSend(UartStr_TypeDef *_pUart, uint8_t *_ucaBuf, uint16_t _usLen);
static uint8_t UartGetChar(UartStr_TypeDef *_pUart, uint8_t *_pByte);
static void UartIRQ(UartStr_TypeDef *_pUart);
...
*********************************************************************************************************
*/
UartStr_TypeDef *ComToUart(UartPort_TypeDef _ucPort)
{
	if (_ucPort == COM1)
	{
		#if UART1_FIFO_EN == 1
...
*********************************************************************************************************
*/
USART_TypeDef *ComToUSARTx(UartPort_TypeDef _ucPort)
{
	if (_ucPort == COM1)
	{
		#if UART1_FIFO_EN == 1
